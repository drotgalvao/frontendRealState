...
export const CreateListing = () => {
  ...
  const [imagemUploadError, setImagemUploadError] = useState(false);
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((error) => {
          console.log(error);
          setImagemUploadError("Image upload failed (2 mb max per image)");
          setUploading(false);
        });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(+formData.price < +formData.discountPrice){
        return setError("Discount price must be less than price");
      }
      setLoading(true);
      setError(false);
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <p className="text-red-700 text-sm">
            {imagemUploadError && imagemUploadError}
          </p>
